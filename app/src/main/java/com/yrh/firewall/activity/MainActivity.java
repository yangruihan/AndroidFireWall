...
import com.yrh.firewall.adapter.PagerViewAdapter;
...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private DialogPlus mDialogPlusIncommingBlackList;
    private RecyclerView mRecyclerViewIncommingBlackList;

    // SMSblacklist 组件
    private PhoneInfoAdapter mSMSBlackListPhoneInfoAdapter;
    private DialogPlus mDialogPlusSMSBlackList;
    private RecyclerView mRecyclerViewSMSBlackList;

    // Settings 组件
    private Switch mSwitchIncommingBlackList;
    private Switch mSwitchSMSBlackList;
    ...
    private void initView() {
        ...
        mSwitchIncommingBlackList = (Switch) settingsView.findViewById(R.id.switchIncommingBlackList);
        mSwitchIncommingBlackList.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                Intent intent = new Intent(MainActivity.this, IncommingBlackListService.class);
                if (isChecked) {
                    startService(intent);
                } else {
                    stopService(intent);
    ...
    private void initRecyclerView(View view, int resourceId) {
        ...
        if (resourceId == R.id.recyclerViewIncommingBlackList) {
            mRecyclerViewIncommingBlackList = recyclerView;
            ...
            mRecyclerViewSMSBlackList = recyclerView;
        ...
        switch (v.getId()) {
            case R.id.fab: {
                if (mViewPager.getCurrentItem() == PAGE_INCOMMINGBLACKLIST) {
                    mDialogPlusIncommingBlackList = getIncommingBlackListDialog();
                    mDialogPlusIncommingBlackList.show();
                } else if (mViewPager.getCurrentItem() == PAGE_SMSBLACKLIST) {
                    mDialogPlusSMSBlackList = getSMSBlackListDialog();
                    mDialogPlusSMSBlackList.show();
                }
                break;
            }
        }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == INTENT_ADD_PHONE_NUM) {
            if (mDialogPlusIncommingBlackList != null && mDialogPlusIncommingBlackList.isShowing()) {
                mDialogPlusIncommingBlackList.dismiss();
            }

            if (mDialogPlusSMSBlackList != null && mDialogPlusSMSBlackList.isShowing()) {
                mDialogPlusSMSBlackList.dismiss();
            }
                ...
                if (b != null) {
                    ...
                    if (b.getInt("inIncommingBlack") == 1) {
                        if (mIncommingBlackListPhoneInfoAdapter != null) {
                            mRecyclerViewIncommingBlackList.smoothScrollToPosition(mRecyclerViewIncommingBlackList.getTop());
                            mIncommingBlackListPhoneInfoAdapter.addDate(phoneInfo);
                        }
                    }
                    if (b.getInt("inSMSBlack") == 1) {
                        if (mSMSBlackListPhoneInfoAdapter != null) {
                            mRecyclerViewSMSBlackList.scrollToPosition(0);
